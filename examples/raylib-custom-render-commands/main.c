...
//    currentTime = GetTime();
    ...
    Custom_Raylib_Render(renderCommands, fonts);
    EndDrawing();
...
int main(void) {
    ...
    render_texture_camera = (Camera) { 0 };
    render_texture_camera.position = (Vector3) { 0.0f, 0.0f, 140.0f }; // Camera position
    render_texture_camera.target = (Vector3) { 0.0f, 0.0f, 0.0f };      // Camera looking at point
    render_texture_camera.up = (Vector3) { 0.0f, 1.0f, 0.0f };          // Camera up vector (rotation towards target)
    render_texture_camera.fovy = 45.0f;                                // Camera field-of-view Y
    render_texture_camera.projection = CAMERA_PERSPECTIVE;             // Camera mode type

    render_defered_camera = render_texture_camera;
    render_model_camera = render_texture_camera;
    ...
    Font fonts[2];
    fonts[FONT_ID_BODY_24] = LoadFontEx("resources/RobotoMono-Medium.ttf", 48, 0, 400);
	SetTextureFilter(fonts[FONT_ID_BODY_24].texture, TEXTURE_FILTER_BILINEAR);
    fonts[FONT_ID_BODY_16] = LoadFontEx("resources/RobotoMono-Medium.ttf", 32, 0, 400);
    SetTextureFilter(fonts[FONT_ID_BODY_16].texture, TEXTURE_FILTER_BILINEAR);
    Clay_SetMeasureTextFunction(Raylib_MeasureText, fonts);

    //--------------------------------------------------------------------------------------
    RenderTexture renderTexture = LoadRenderTexture(GetScreenWidth(), GetScreenHeight());
